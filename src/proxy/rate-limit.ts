...
// Code ported from Khanon, thanks o/ 
// Just modified bit as i don't have the image models support yet ._. 

export const AGNAI_DOT_CHAT_IP = new Set([
  // Agnai.chat
  "157.230.249.32", // old
  "157.245.148.56",
  "174.138.29.50",
...
const getTryAgainInMs = (ip: string, limit: number) => {
  const now = Date.now();
...
const getStatus = (ip: string, limit: number) => {
  const now = Date.now();
  const attempts = lastAttempts.get(ip) || [];
  const validAttempts = attempts.filter(isRecentAttempt(now));
...
export const ipLimiter = async (
  ...
  const limit = req.user?.rateLimit || config.modelRateLimit;
  ...
  if (AGNAI_DOT_CHAT_IP.has(req.ip)) {
  ...
  const { remaining, reset } = getStatus(rateLimitKey, limit);
  res.set("X-RateLimit-Limit", limit.toString());
  res.set("X-RateLimit-Remaining", remaining.toString());
  res.set("X-RateLimit-Reset", reset.toString());

  const tryAgainInMs = getTryAgainInMs(rateLimitKey, limit);
  if (tryAgainInMs > 0) {
    res.set("Retry-After", tryAgainInMs.toString());
    res.status(429).json({
      error: {
        type: "proxy_rate_limited",
        message: `Proxy is rate limited to ${limit} prompts per minute. Please try again in ${Math.ceil(
          tryAgainInMs / 1000
        )} seconds.`,
      },
    });
