...
export type OpenAIModel = "gpt-3.5-turbo" | "gpt-4" | "gpt-4-32k" | "gpt-4-turbo" | "gpt-o";
...
export class OpenAIKeyProvider implements KeyProvider<OpenAIKey> {
  ...
  public get(model: Model, applyRateLimit: boolean = true) {
    const needGpt4Omini = model.startsWith("gpt-4o-mini");
    const needGpt4 = model.startsWith("gpt-4") || model.startsWith("chatgpt");
	const needGpt432k = model.startsWith("gpt-4-32k");
	const needGptO = model.startsWith("o1-") || model.startsWith("o3-");
	const needGpt4Turbo = model.startsWith("gpt-4-1106") || model.startsWith("gpt-4-vision-preview");
    ...
    (key) =>
        ...
        !key.isSpecial &&
        (!config.disabledGptModels || !config.disabledGptModels.includes(model))
);
	

    let availableSpecialKeys = this.keys.filter((key) => {
      return (
          !key.isDisabled &&
          key.isSpecial &&
          key.specialMap && // Check specialMap
          key.specialMap.hasOwnProperty(model)
      );
  });
	...
	if (availableKeys.length === 0 && isSpecialSelected == false) {
	  let message = needGpt4
		? `No ${model} keys available. Try selecting other model.`
		: `No active OpenAI ${model} keys available.`;
	  throw new Error(message);
	}


    // fuck i should be able to do simple operations but i'm fucking cooked today
    if (!needGpt4Omini && config.turboOnly) {
      if (config.turboOnly && needGpt4) {
        throw new Error(
          "Proxy has disabled most of GPT-4 models, select 3.5 model or 4o-mini model."
        );
      }
    }
  ...
  public getLockoutPeriod(model: Model = "gpt-4"): number {
    ...
    const needGpt4 = needGpt432k && needGptO ? false : (model.startsWith("gpt-4") || model.startsWith("chatgpt"));
	...
	if (needGpt432k) {
	  ...
	  activeKeys = this.keys.filter(
         (key) => !key.isDisabled && (!needGptO || key.isGptO)
      );
	} else {
		activeKeys = this.keys.filter(
