...
router.get("/view-key/:key", (req, res) => {
  const key = keyPool.getKeyByHash(req.params.key);
  if (!key) {
    return res.status(404).send("Key not found");
  }
  res.render("admin/view-key", { "key": key });
});
...
router.post("/edit-user/:token", (req, res) => {
  ...
  if (edit_type == "Google") {
	user.allowGoogle = Boolean(edit_value);
  }
...
router.get("/key-manager", (req, res) => {
  const sort = parseSort(req.query.sort) || ["org", "isGpt4", "isGpt432k", "isGptO", "isOverQuota","isRevoked"];
  const hide = parseHide(req.query.sort) || ["org", "isGpt4", "isGpt432k", "isGptO", "isOverQuota","isRevoked"];
