...
type CreateFn = typeof DeepseekKeyProvider.prototype.createKey;


export class DeepseekKeyChecker {
  private readonly keys: DeepseekKey[];
  ...
  constructor(keys: DeepseekKey[], updateKey: UpdateFn, createKey: CreateFn) {
    this.keys = keys;
  ...
  private async checkKey(key: DeepseekKey) {
  ...
  private async testLiveness(key: DeepseekKey): Promise<{ rateLimit: number }> {
	const payload = {
	  model: "deepseek-chat",
	  max_tokens: 1,
	  messages: [{ role: "user", content: "hi" }],
	};
	const { headers, data } = await axios.post<DeepseekError>(
