...
import type { TogetherKey, TogetherKeyProvider } from "./provider";
...
const POST_CHAT_COMPLETIONS_URL = "https://api.together.xyz/v1/chat/completions";
const GET_MODELS_URL = "hhttps://api.together.xyz/v1/models";
...
type UpdateFn = typeof TogetherKeyProvider.prototype.update;
type CreateFn = typeof TogetherKeyProvider.prototype.createKey;


export class TogetherKeyChecker {
  private readonly keys: TogetherKey[];
  private log = logger.child({ module: "key-checker", service: "together" });
  ...
  constructor(keys: TogetherKey[], updateKey: UpdateFn, createKey: CreateFn) {
    this.keys = keys;
  ...
  private async checkKey(key: TogetherKey) {
  ...
  private handleAxiosError(key: TogetherKey, error: AxiosError) {
    if (error.response && TogetherKeyChecker.errorIsTogetherError(error)) {
  ...
  private async testLiveness(key: TogetherKey): Promise<{ rateLimit: number }> {
	const payload = {
	  model: "google/gemma-2b-it",
	  max_tokens: 1,
	  messages: [{ role: "user", content: "hi" }],
	};
	const { headers, data } = await axios.post<TogetherError>(
  ...
  static errorIsTogetherError(
    error: AxiosError
  ): error is AxiosError<TogetherError> {
