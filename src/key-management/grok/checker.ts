...
import type { GrokKey, GrokKeyProvider } from "./provider";
...
type UpdateFn = typeof GrokKeyProvider.prototype.update;
type CreateFn = typeof GrokKeyProvider.prototype.createKey;


export class GrokKeyChecker {
  private readonly keys: GrokKey[];
  private log = logger.child({ module: "key-checker", service: "grok" });
  ...
  constructor(keys: GrokKey[], updateKey: UpdateFn, createKey: CreateFn) {
    this.keys = keys;
  ...
  private async checkKey(key: GrokKey) {
  ...
  private handleAxiosError(key: GrokKey, error: AxiosError) {
    if (error.response && GrokKeyChecker.errorIsGrokError(error)) {
  ...
  private async testLiveness(key: GrokKey): Promise<{ rateLimit: number }> {
	const payload = {
	  model: "grok-2-1212",
	  max_tokens: 1,
	  messages: [{ role: "user", content: "hi" }],
	};
	const { headers, data } = await axios.post<GrokError>(
  ...
  static errorIsGrokError(
    error: AxiosError
  ): error is AxiosError<GrokError> {
