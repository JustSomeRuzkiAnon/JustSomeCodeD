...
export const Deepseek_SUPPORTED_MODELS: readonly DeepseekModel[] = [
  "deepseek-chat",
  "deepseek-coder",
  "deepseek-reasoner"
] as const;
...
export class DeepseekKeyProvider implements KeyProvider<DeepseekKey> {
  ...
  constructor() {
    const keyString = config.deepseekKey?.trim();
    if (!keyString) {
      this.log.warn("DEEPSEEK_KEY is not set. Deepseek API will not be available.");
      return;
    }
    let bareKeys: string[];
  ...
  public getHashes() {
	  this.checker = new DeepseekKeyChecker(this.keys, this.update.bind(this), this.createKey.bind(this));
	  let x: string[] = [];
  ...
  public get(model: Model, applyRateLimit: boolean = true) {
	...
	if (selectedKey == undefined) {
		let message = "No active Deepseek keys available.";
  ...
  public update(keyHash: string, update: DeepseekKeyUpdate) {
   ...
   */
  public getLockoutPeriod(model: Model = "deepseek-chat"): number {
  ...
  public updateRateLimits(keyHash: string, headers: http.IncomingHttpHeaders) {
	...
	if (!requestsReset && !tokensReset) {
	  this.log.warn(
		{ key: key.hash },
		`No rate limit headers in Deepseek response; skipping update`
	  );
	  return;
	}
