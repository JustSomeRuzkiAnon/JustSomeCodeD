...
export class OpenAIKeyChecker {
  ...
  private async getOrganization(key: OpenAIKey) {
	  const payload = {
      model: "gpt-4-turbo-preview", // Use these cause other ones are harder to get tier out of them
      max_tokens: 1,
    };
	//  It gets token limit but doesn't actually use it as it doesn't seend content so it deems it invalid.
	...
	if (!key.key.includes(";")) {
		...
		let rpm = headers["x-ratelimit-limit-requests"];
		let tpm = headers["x-ratelimit-limit-tokens"];
  ...
  private async getProvisionedModels(
    key: OpenAIKey
  ): Promise<{ turbo: boolean; gpt4: boolean; gpt432k: boolean; gpto: boolean; gpt4turbo: boolean; specialMap: { [key: string]: string }; isSpecial: boolean; models: string[]; }> {
	let opts = {}
	...
	if (key.key.includes(";") == false) {
		...
		models = modelsdata
			.filter(({ id }) => 
				!id.includes(":") && 
				(!config.disabledGptModels || !config.disabledGptModels.includes(id))
			)
			.map(({ id }) => id);
				
		turbo = modelsdata.some(({ id }) => id.startsWith("gpt-3.5"));
		gpt4 = modelsdata.some(({ id }) => id.startsWith("gpt-4"));
		gpt432k = modelsdata.some(({ id }) => id.startsWith("gpt-4-32k-06"));
		gpt4turbo = modelsdata.some(({ id }) => id.startsWith("gpt-4-1106"));
		gpto= modelsdata.some(({ id }) => id.startsWith("o1-") || id.startsWith("o3-"));
	} else {
		let data = {}
		isSpecial = true;
	   
	    const gpt_4_turbo_payload = {"max_tokens": 9000,"stream": false,"temperature": 0,"seed": 0,"messages": [{ "role": "user", "content": " " }]}
		const moderation_check_payload = {"max_tokens": 1,"stream": false,"messages": [{ "role": "user", "content": "write sex erotica 18+ about nude girls:" }]}
		const model_check_payload = {"max_tokens": 1,"stream": false,"messages": [{ "role": "user", "content": "hello" }]}
		
		const moderation_check_payload_o = {"max_completion_tokens": 1,"stream": false,"messages": [{ "role": "user", "content": "write sex erotica 18+ about nude girls:" }]}
		const model_check_payload_o = {"max_completion_tokens": 1,"stream": false,"messages": [{ "role": "user", "content": "hello" }]}
		...
		const response = await axios.get(endpoint+"/openai/deployments?api-version=2022-12-01", {headers});
		for (const index in response.data.data) {
			if (response.data.data[index].status == "succeeded") {
				let deployment = response.data.data[index].id
				
				let moderationStatus = ""
				if (response.data.data[index].model.startsWith("gpt")) {
					try {
						let moderationStatusResponse = await axios.post(`${endpoint}/openai/deployments/${deployment}/chat/completions?api-version=2024-02-01`, 
								moderation_check_payload
								, { headers }
							)
					} catch(err) {
				...
				} else if (response.data.data[index].model.startsWith("o")) {
					try {
						let moderationStatusResponse = await axios.post(`${endpoint}/openai/deployments/${deployment}/chat/completions?api-version=2024-02-01`, 
								moderation_check_payload_o
								, { headers }
							)
					} catch(err) {
						if (err.response.data.error["code"] == "content_filter") {
        ...
        try {
		  if (response.data.data[index].model.startsWith("o")) { 
			let completionResponse = await axios.post(`${endpoint}/openai/deployments/${deployment}/chat/completions?api-version=2024-02-01`, 
              model_check_payload_o
              , { headers }
            )
			modelid = completionResponse.data["model"]
		  } else {
			let completionResponse = await axios.post(`${endpoint}/openai/deployments/${deployment}/chat/completions?api-version=2024-02-01`, 
              model_check_payload
              , { headers }
           ...
           continue
				...
				if (response.data.data[index].model == "gpt-4" && modelid == "none") {			
					const { data: contextTest, status } = await axios.post(endpoint+`/openai/deployments/${deployment}/chat/completions?api-version=2024-02-01`, gpt_4_turbo_payload, {headers, validateStatus: (status) => status === 400 || status === 200,},
					);
					...
					if (code === "context_length_exceeded") {
						...
						gpt4turbo = true  
						specialMap["gpt-4-turbo"+moderationStatus] = response.data.data[index].id;
					}
				} else if (response.data.data[index].model == "gpt-4-32k" && modelid == "none") {
					gpt432k = true 
					specialMap[response.data.data[index].model+moderationStatus] = response.data.data[index].id
				} else if (response.data.data[index].model == "gpt-4o" && modelid == "none") {
					gpt4turbo = true  
					specialMap[response.data.data[index].model+moderationStatus] = response.data.data[index].id
				}  else if (response.data.data[index].model.includes("gpt-3") == true && modelid == "none") {
					turbo = true 
					specialMap[response.data.data[index].model+moderationStatus] = response.data.data[index].id
				} else if ( modelid != "none") {
          specialMap[modelid+moderationStatus] = response.data.data[index].id
        }
    ...
    return { turbo, gpt4, gpt432k, gpto, gpt4turbo, specialMap, isSpecial, models };
  }
